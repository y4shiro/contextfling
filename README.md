...
v0.1.1 の失敗経路を 68 tests で検証しています。PR #13 の 2026-08-26 修正前 build では foreground / background とも prompt 挿入後の自動送信と clipboard fallback が失敗しました。原因修正後の HEAD `5cf1416` を Chrome 実機で再確認した結果、foreground は送信成功、background は hidden document の React / ProseMirror state readiness を保証できず fail-closed しました。background の clipboard fallback 自体は `copied` まで成功していますが、自動送信は成立していません。修正前後とも自動 retry と二重送信は発生していません。background 自動送信を撤回し foreground-only とする option 2 と、予期しない失敗時の option 5（no-op + 明示的 feedback）を [ADR 0003](docs/adr/0003-background-chatgpt-handoff-withdrawal.md) で Proposed のまま検討中です。option 3（background paste-only）は次点候補ですが、clipboard 上書き・手動貼り付け・分岐保守の判断が残っています。仕様変更は未実装で、人手の採択待ちです。ChatGPT Web の DOM に依存する Experimental 機能であり、Chrome Web Store にはまだ公開していません。v0.1.0 は `about:blank` 完了イベントの既知 race を含むため非推奨です。
