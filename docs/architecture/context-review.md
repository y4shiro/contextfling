...
> 現状更新: v0.1.1 実装済み。PR #13 の修正前 build では foreground / background とも自動送信と clipboard fallback が失敗した。HEAD `5cf1416` の re-smoke では foreground の送信成功、background hidden の fail-closed、clipboard DOM copy の成功を確認した。background 自動送信の撤回候補は Proposed のまま人手判断待ち。Chrome Web Store 未公開。
...
- PR #13 の修正前 build では foreground / background とも prompt 挿入後に自動送信されず、clipboard fallback も `write-failed` で失敗した。段落 plain-text 復元と単回 offscreen DOM copy を実装し、68 tests で検証済み。HEAD `5cf1416` の re-smoke では foreground の送信成功、background hidden の composer write gate failure、clipboard DOM copy の成功を確認した。background hidden の React / ProseMirror readiness は保証できないため、option 2 の foreground-only を推奨候補とし、option 3 の background paste-only は次点とする。仕様変更は未実装で、人手判断待ち。logged-out、selector 変更、tab close、同意撤回、追加 Security / Privacy review は未完了。
...
1. composer の段落 plain-text 復元と単回 offscreen DOM copy の最小修正、および 68 tests の結果を維持する。
...
3. ADR 0003 の Proposed 比較を人手で判断する。現時点の推奨は option 2 foreground-only、予期しない失敗は option 5 no-op + 明示的 feedback、option 3 background paste-only は次点とする。仕様変更は採択まで実装しない。
4. Smoke 結果と permission、Privacy、Security、test fixture、acceptance criteria を同期し、Release Gate として logged-out、DOM 変更、tab close、同意撤回、正式名称、CWS listing、Privacy URL、サポート窓口を確定する。
5. 撤回条件を採択した場合は DOM automation を foreground-only / paste-only / no-op の選択に合わせ、設定 UI、schema、tests、ADR と関連文書を同じ変更で更新する。
